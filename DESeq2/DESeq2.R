...
barplot(go_bp, showCategory = 10, title = "GO Biological Process")

barplot(go_mf, showCategory = 10, title = "GO Molecular Function")

barplot(go_cc, showCategory = 10, title = "GO Cellular Component")

# Save the GO results to CSV
write.csv(as.data.frame(go_bp), "GO_Biological_Process.csv")
write.csv(as.data.frame(go_mf), "GO_Molecular_Function.csv")
write.csv(as.data.frame(go_cc), "GO_Cellular_Component.csv")

cat("GO analysis results saved to CSV files.\n")
